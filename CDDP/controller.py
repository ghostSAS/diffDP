...
class DDP:
    ...
    def fit(self, x0, us_init, dt_init, maxiters = 100, early_stop = True):
        '''
        Args:
          x0: initial state
        ...
        '''
        return run_ddp(self.dynamics.f, self.dynamics.f_prime, self.cost.L,
                        self.cost.Lf, self.cost.L_prime, self.cost.Lf_prime,
                        x0, us_init, dt_init, maxiters, early_stop, **self.params)

    def rollout(self, x0, us, dts):
        '''
        Args:
          x0: initial state
        ...
        '''
        return rollout(self.dynamics.f, self.cost.L, self.cost.Lf, x0, us, dts)


@numba.njit
def run_ddp(f, f_prime, L, Lf, L_prime, Lf_prime, x0, u_init, dt_init, max_iters, early_stop,
    ...
    xs, J_old = rollout(f, L, Lf, x0, us, dts)
    # cost trace
    cost_trace = [J_old]

    # Run main loop
    for it in range(max_iters):
        ks, Ks, exp_cost_redu = backward_pass(f_prime, L_prime, Lf_prime, xs, us, dts, regu)
        ...
        for alpha in alphas:
          xs_new, us_new, J_new = forward_pass(f, L, Lf, xs, us, dts, ks, Ks, alpha)
          if J_old - J_new > 0:
...
@numba.njit
def forward_pass(f, L, Lf, xs, us, dts, ks, Ks, alpha):
    '''
       Forward Pass
    '''
    ...
    for n in range(us.shape[0]):
        ...
        xs_new[n+1] = RK4(dts[n], xs_new[n], us_new[n], f)
...
@numba.njit
def backward_pass(f_prime, L_prime, Lf_prime, xs, us, dts, regu):
    '''
       Backward Pass
    '''
    ...
    for n in range(us.shape[0] - 1, -1, -1):
        ...
        f_x, f_u = f_prime(xs[n], us[n], dts[n])
